...
JSONEditor.prototype = {
  ...
  _loadExternalRefs: function(schema, callback) {
    ...
    $each(refs,function(url) {
      ...
      const exturl = schema.baseUrl + '/' +url;
      r.open("GET", exturl, true);
      r.onreadystatechange = function () {
        ...
        if(r.status === 200) {
          ...
          response.baseUrl = schema.baseUrl;
          
          // update the baseUrl if the $ref is in different directory.
          const lastSlash = url.lastIndexOf("/");
          if (lastSlash>-1) {
              if (url.startsWith('/')) {
                  // reset the baseUrl accorting to the absolute path
                  response.baseUrl = url.substring(0, lastSlash);
              } else {
                  // relative path of the current $ref
                  response.baseUrl += '/' + url.substring(0, lastSlash);
              }
          }
...
JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    // If the object should be rendered as a div
      ...
      this.toggle_button = this.getButton('Hide Structure','collapse',this.translate('button_collapse'));
      ...
      this.toggle_button.addEventListener('click',function(e) {
        ...
        if(self.collapsed) {
          ...
          self.setButtonText(self.toggle_button,'Hide Structure','collapse',self.translate('button_collapse'));
        }
        else {
          self.editor_holder.style.display = 'none';
          self.collapsed = true;
          self.setButtonText(self.toggle_button,'Show Structure','expand',self.translate('button_expand'));
        }
      });
      ...
      this.addproperty_button = this.getButton('Optionals','edit','Object Properties');
