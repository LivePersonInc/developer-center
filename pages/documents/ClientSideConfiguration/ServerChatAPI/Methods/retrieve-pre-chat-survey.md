...
| surveyApiId | Requests a specific survey by API ID. | alphanumeric | If the surveyName is provided, this parameter will be ignored. To retrieve the surveyApiId, you need to call GET https://[{domain}](/agent-domain-domain-api.html)/api/account/{accountId}/configuration/le-campaigns/campaigns/{campaignId}/engagements/{engagementId}?v=3.4 (Bearer token required) to get the `windowID` from the current engagement information. Next, you need to call GET https://[{domain}](/agent-domain-domain-api.html)/api/account/{accountId}/configuration/engagement-window/window-confs/{windowId} (Bearer token required) to get the `surveyPreChatId` from the window information. The `surveyPreChatId` is the value that needs to be passed through this parameter.
