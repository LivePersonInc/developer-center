...
* `identities` - Each identity consists of 3 parameters: `iss` - Issues, who identified the consumer - usually the brand; `acr` - Account Config Read, means the level of authentication (usually loa1); `sub` - Subject, the identifier (e.g, email, phone number, user name supplied by the brand).
The definition should comply with the RFC - http://openid.net/specs/openid-connect-core-1_0.html#IDToken
The identities are considered when choosing an elidible engagement - the identity strength and whether or not the specific identity has an open convesation. 
