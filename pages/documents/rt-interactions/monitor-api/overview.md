...
* `identities` - identification indication to be considered when choosing engagement. Each identity consists of 3 parameters: `iss` - Issues, who identified the consumer - usually the brand; `acr` - Account Config Read, means the level of authentication (usually loa1); `sub` - Subject, the identifier (e.g, email, phone number, user name supplied by the brand)
