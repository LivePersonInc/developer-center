...
**Request Headers**
 ...
 |LP-stop-if-certificate-not-found|    If cannot match certificate to supplied parameters will not forward the request (instead will return INTERNAL_SERVER_ERROR with the reason), **Default value is false (proxy request even if certificate not found). Not Required**. |
