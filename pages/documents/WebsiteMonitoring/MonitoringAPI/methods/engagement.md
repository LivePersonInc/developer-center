...
Note (!) Specifically in the case of a "500 - Loading account" response (as shown in the response entity examples above), it is important to retrieve the value of the 'vid' from the response body and append it as the value of the 'vid' query param for the retry request (to be issued following an pause interval of a few seconds).