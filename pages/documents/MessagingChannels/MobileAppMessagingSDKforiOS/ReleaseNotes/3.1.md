...
Campaign includes Engagement info which allows to control the consumer's routing. Note: sessionID and visitorID are optional parameters.  If these values are not used or left as nil then the SDEs will only be sent for routing and they will not appear to the agent. If you add them into the constructor then the SDEs will be visible to the agent.</td>
 </tr>
 <tr>
