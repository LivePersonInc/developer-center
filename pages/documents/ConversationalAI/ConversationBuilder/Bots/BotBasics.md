...
- **Enable Bot**: Use this Online/Offline setting to turn the bot on and off, respectively. The default value is Online. When the bot is online, it responds to user messages. When it is offline, it doesn't. Note that this setting doesn't affect the agent connector (created during [deployment](conversation-builder-testing-deployment-deploying-to-liveengage.html)); this setting turns off the bot itself.
