...
2. Configure the integration settings (required fields are marked with asterisks):
    ...
    - **Agent Id**: Optional. Specify the ID (login name) of the human agent or the bot ID of the bot to which to transfer the conversation. (The bot ID can be obtained from a bot's [bot settings](conversation-builder-bots-bot-basics.html#configure-bot-settings).) Transfer of the conversation to this agent (human or bot) only occurs if the agent is assigned to the skill ID that you specify and is available; otherwise, transfer to the skill ID occurs instead.
