...
2. Configure the integration settings (required fields are marked with asterisks):
    ...
    - **Tango Context**: Used for bot-to-bot escalations (transfers) only. Select this to enable the use of the Tango Context object that is sent from the sender bot to the receiver bot during the transfer. The Tango Context object can contain an intent ID or user message (set by you in the integration interaction), so the receiver bot knows the appropriate dialog to start after the transfer. For more information, see [here](conversation-builder-bots-bot-to-bot-transfers.html).
