...
2. Configure the integration settings (required fields are marked with asterisks):
    ...
    - **Bot Transfer Context**: Used for bot-to-bot escalations (transfers) only. Select this to enable your use of the Bot Transfer Context object that is sent from the sender bot to the receiver bot during the transfer. Once enabled, before calling the integration, you can use scripting functions to set an intent ID or user message in the object. This lets the receiver bot know the appropriate dialog to start after the transfer. For more on this and bot-to-bot transfers in general, see [here](conversation-builder-bots-bot-to-bot-transfers.html).
