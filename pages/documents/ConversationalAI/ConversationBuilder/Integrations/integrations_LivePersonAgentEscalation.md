...
2. Configure the integration settings (required fields are marked with asterisks):
    ...
    - **Agent Skill Id**: Specify the ID of the skill to which to transfer the conversation. The skill is defined in LiveEngage. Here you can specify the ID using a bot context variable like `{$botContext.skillId}`, or you can enter a direct, numeric value. During bot processing, this value is evaluated; if it isn't numeric, the fallback message is sent to the user.
