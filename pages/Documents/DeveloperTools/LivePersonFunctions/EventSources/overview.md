...
LivePerson Functions may be listening to Conversation Cloud events or are directly linked to conversational bots. Functions will start reacting to events once they are successfully deployed until they do not have an effect.
...
Functions can be integrated with other LivePerson systems through events enabling the tailoring of the conversation cloud to your brand's needs. They are creating flows and functionality that are not available out of the box.

Each event has a specific input (payload) that the triggering system will provide at runtime. Further, we provide a sample payload particular to the event, which will be displayed during the development or in the invocation screen. Currently, the following event sources are supported:
...
Only productive functions will be reacting to such events. Suppose you want to learn more head over [here](liveperson-functions-event-sources-automatic-messages.html).
...
|Messaging Line in Off-Hours|Triggered when a conversation was *open* during office hours. And a new message is **written** during off-hours (essentially when a consumer sends an off-hour message).|Controller Bot (Messaging Automatic Messages)|1|✅|
...
[Post conversation Survey Bots](conversation-builder-bots-post-conversation-survey-bots.html) collect consumer feedback at the conversation's end. This integration with Functions occurs when the survey starts and ends. A Survey Bot can be configured in the Conversation Builder. Please see details [here](liveperson-functions-event-sources-post-conversation-survey.html).
...
Conversational Commands allow agents to invoke functions in the Agent Workspace conversation. please see details [here](liveperson-functions-event-sources-conversational-commands.html).
...
Third-party bots are managed through LivePerson's Conversational Cloud and can be displayed as a human agent or a LivePerson bot. You can leverage it with Functions to build custom integrations with another third-party bot provider. Please see details [here](liveperson-functions-event-sources-third-party-bots.html).
...
There is only a single event of Chat Post Survey that uses Functions to re-create one of our Legacy features, the ability to send out transcripts after a chat conversation ends conditionally. Please see details [here](liveperson-functions-event-sources-chat-post-survey.html).
