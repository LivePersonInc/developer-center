...
3. Under **Add KB**, add the API Access Key that was copied from the Knowledge Base portal in an earlier step. This needs to be provided in order to discover the knowledge bases that have been created when configuring article recommendations. Click **Save**.
4. Follow the similar steps that were taken with bots: Enable the Airline FAQ knowledge base under the **Configure article recommendations** section and assign the “agent” skill to it.

    <img class="fancyimage" style="width:800px" src="img/agentassisttutorial/config_article_recommendations.png">

{: .important}
**Customize your POC:** If you built out additional resources in previous steps, make sure to have those resources enabled as well to pull recommendations from for your users. Depending on the number of resources, it may be useful to increase the number of recommendations that you display to your agents when they are in a conversation.
...
3. Give the campaign the name "Agent Assist Testing," and under **Campaign goal**, add the default “Interact with consumers.” 
...
7. Back on the "Agent Assist Testing" page, confirm that the skill being routed to is "agent." If so, select the orange **Publish** button in the upper-right corner. Once published, there will be a visible Running icon <img style="width:30px" src="img/agentassisttutorial/icon_running.png"> to the right of the engagement.
