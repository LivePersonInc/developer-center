...
| Target | Applies when the Action Type equals “Web URL." Select whether to open the URL in a new window, the current window, or a slideout window. The slideout window "slides out" from the conversation window. For an attractive and tightly integrated experience for the consumer, use the "Slideout" option. | Required | New Window |
