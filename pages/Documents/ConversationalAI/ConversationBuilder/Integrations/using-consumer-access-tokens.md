...
By [configuring Consumer Authentication credentials](conversation-builder-integrations-using-consumer-access-tokens.html#configure-consumer-authentication-credentials), The consumer is sent an authentication link, uses it to authenticate, and obtains a token that is sent to the bot. This “delegates” access to the bot, so it can make the API calls.  
...
**To add a Consumer Authentication credentials**
