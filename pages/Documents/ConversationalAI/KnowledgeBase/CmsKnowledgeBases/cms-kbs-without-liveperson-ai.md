...
5. Set up the on-demand content retrieval using your CMS' query and answer API. Somewhere in the request (request parameters, post body, etc.), you must pass the input phrase to use when performing the search in the CMS. The request should return the best article matches. Each article must contain a title and at least one of these content attributes: summary, detail, content URL, image URL, video URL, or audio URL.
    ...
    * Click **Map Content Metadata**, and map the article suggestions/answers to the LivePerson Knowledge Base data model. For help with this, see [here](knowledge-base-cms-knowledge-bases-mapping-content-metadata.html).
7. Click **Save**.
