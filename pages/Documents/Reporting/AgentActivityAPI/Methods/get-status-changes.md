...
### Request
{: .notice}
**v1**: Each page in the API response is limited to 20 state changes * limit parameter * query days. For example, if the limit is set to 10 (agents), 1-day query then limitation per page = 200 state changes.
...
| --- | --- | --- | --- | --- | --- |
| V | Version of the API (1 or 2) | String | No | The default is 1 |  **v1**: returns the records grouped by an agent with a limitation of the number of records in every page (see the note above). </br> **v2**: Returns the records in a flat structure and not grouped by an agent. No limitation on the number of records in every page |
...
| sessionId | Agent unique session |  | sessionId is currently missing from the first 2 events. To link these 2 events to a session, you will need to look for the following event, based on its timestamp and where is sequence =2|
| prevStatusChangeTime |  |  | Currently missing from the logout event and from the default status that is set after login |
| sequenceNumber |  |  | Login event will always have a sequnceNumber = -1. </br> The system default state after login will always have a sequenceNumber = 1.|
