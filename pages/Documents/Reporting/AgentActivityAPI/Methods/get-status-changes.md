...
For example, if the limit is set to 10 (agents), 1 day query then limitation per page = 200 state changes.<br/> In case of an excessive state changes, some of the states/agents may be truncated based on the maximum states limitation.<br/>This has been updated on [v2] (agent-activity-api-methods-get-status-changes-v2.html) where all states are returned in a flat response (not grouped by agent).
